...
class QuantizeEnv:
    def __init__(self, model, pretrained_model, compress_ratio, args, n_data_worker=16,
        ...
        self.strategy_actor = [0 for i in range(self.n_quantizable_layer)]
    ...
    def step(self, action, action_actor):
        ...
        # if self.strategy_actor[self.cur_ind] != action_actor:
        self.strategy_actor[self.cur_ind] = action_actor
            #  self.eval_actor = True
    ...
    def _create_record(self):
        ...
        centroids = []
        labels = []
        bar = Bar('k-means:', max=len(self.quantizable_idx) * (self.max_bit - self.min_bit + 1))
        for i, layer in enumerate(self.model.modules()):
            if i not in self.quantizable_idx:
                continue
            layer_c=[]
            layer_l=[]
            for j in range(self.min_bit, self.max_bit + 1):
                w = layer.weight.data
